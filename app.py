...
if st.button("✅ アドバイスを生成する", type="primary", key="generate_main"):
    ...
    st.download_button(
        "Markdownをダウンロード",
        data=advice.encode("utf-8"),
        file_name=f"pain_advice_{ts}.md",
        mime="text/markdown",
        key="dl_md"
    )
    ...
    row = {
        ...
        "free_text": (free_text or "").replace("\n"," / "),
    ...
    try:
        with LOG_PATH.open("a", encoding="utf-8", newline="") as f:
            ...
            w.writerow(row)
        st.success(f"ログ保存: {LOG_PATH.resolve()}")
...
# ================= 参考（折りたたみ：要点まとめ） =================
with st.expander("📚 参考：各テーマの要点（配信用メモ）", expanded=False):
    st.markdown("""
- **股関節の“つまり”**：前方下部＝前傾＋内旋／前方上部＝前傾＋外旋／後方＝後傾＋外旋＋前方シフト。まず骨盤傾き修正→荷重側へ骨盤回旋を同期。
- **反張膝**：微屈で使う（床反力線が膝後方→衝撃吸収↑）。ヒップリフト・前ももの遠心・立位統合。
- **リブフレア**：最優先は「吐き切る」。胸椎は腹圧を保ったまま伸展。下部肋骨と骨盤・恥骨ラインを垂直に。
- **股関節伸展**：座りすぎで屈筋短縮＋臀筋不活動。大腿直筋/腸腰筋ストレッチ→腹圧維持ランジ→ブリッジ。
- **首肩こり**：胸郭位置→肩甲骨（前鋸筋・僧帽下部）。小胸筋短縮対策、すくめない、肋骨突出を抑える。
- **坐骨神経痛**：赤旗除外→保存療法。体幹協調と仙腸関節の安定、股関節伸展確保、歩行の漸増。
""")
